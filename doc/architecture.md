...
| ap_ext_object_mutator | Combat identity of the earned alpha status: level-scaled hit power dealt (vs anyone) and taken, panic immunity, trophy loot. Before-hit seams + monster_on_loot_init, nothing per frame. Canonical section: Object mutator |
...
conquer_smart(smart_id, faction) calls xsmart_spawn.set_exclusive_spawn_tiered(smart, "ap_conquest", faction, spawn_num), setting faction_controlled + faction so the engine respawn filter (smart_terrain.script:1667) passes ONLY the conqueror's entry - the smart respawns solely the winning faction and suppresses its original LTX spawns. Squad sections come from xsmart_spawn.SQUADS_BY_FACTION (stalker *_sim_squad_novice / advanced / veteran), narrowed by the smart's own danger rung: a smart whose LTX declares spawn_all_weak fields novice squads, spawn_all_normal novice/advanced, spawn_all_hard advanced/veteran, spawn_all_worst veteran only (the map's south-to-north ladder read per smart; no rung declared = full pool). The smart also gets smart.respawn_idle = cfg.mutator_area_conquest_respawn_minutes * 60 (MCM under Mutators > Conquest, default 60 game-min, range 1-720) so it repopulates faster than the ~12h vanilla default while held.
...
ap_ext_object_mutator owns what being an alpha means in combat; ap_ext_tracker owns who is an alpha. Four effects, each with its own MCM toggle (Creatures > Alpha -- the creature sibling of the smart-mutators tab):
...
Lifecycle mirrors the tracker's guards: effects set at alpha_promote (consequence handler), cleared on server_entity_on_unregister (id recycling), rebuilt from ap_ext_tracker.get_alphas() at actor_on_first_update (after the tracker's load sweep) and on option change. Everything is event-driven -- the per-hit cost is a pure-Lua next() early-out while no alphas are alive, and there is no timer and no per-frame work. History: these effects shipped once, were dropped in 5065d1f, and return with the vs-anyone dealt scaling (the dropped version scaled only mutant victims) and without the old global 0.5s throttle (it skipped most hits in any real fight, making the scaling stochastic).

Planned, pending in-game probes: extra sensory effects -- a particle mark on the alpha (game_object start_particles at a bone, vanilla precedent bind_signal_light.script) and tinnitus / blur on the player via xsound / xpp. Which particle, which bone, what intensity, and the trigger are chosen in-game, not from files.
