...
| ap_ext_trade | NPC supply-trader orchestration: reads vanilla `[buy_sell]` plus AP `[ap_buy_sell_keep]` overlay (`mod_gulag_job_trade_buy_sell_ap.ltx`); sells loot, restocks pistol + rifle ammo classes plus the keep list |
...
Four AP flows touch items. Each gates on a vanilla classifier; the only AP-curated list is `[ap_buy_sell_keep]` shipped via `mod_gulag_job_trade_buy_sell_ap.ltx`, an explicit allowlist of PDA / medkit / bandage / grenade sections NPCs treat as useful (drives sell-skip + buy-include in the supply trader flow). The same overlay patches `[buy_sell_supplier]` and `[buy_sell_medic]` to flip vanilla restock=0 to restock=1 for grenades and AI medkits, otherwise the buy loop early-returns on them.
