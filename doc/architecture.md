...
| ap_ext_smart_mutator | Runtime smart terrain mutations, all exclusive spawn: territory conquest (grows an outpost of service NPCs + guards), mutant swarm, mutant infestation. Taken smarts are never retaken (is_taken); swarm/infest share the INFEST_SPECIES allowlist |
...
Decay. cfg.mutator_area_conquest_decay_hours (default 48). Scanner clears expired entries (clear_exclusive_spawn) and restores the pre-conquest respawn_idle captured at inject (entry field respawn_idle_base); the smart returns to its original spawns at its original pace.
...
#### Outpost services (conquest only)

A held conquer smart grows into an outpost: ONE service NPC (trader / barman / mechanic / medic) that trades, repairs, or heals, plus a small guard detail. One service per outpost is a ruling, not a tuning value: a hub-grade assortment plus sibling services at a field smart flattens the hub traders' role, so each outpost specializes and variety lives across the map, not inside one smart. This is conquest's growth flavor -- swarm and infest do not grow services (mutants do not build). Gated by cfg.mutator_area_conquest_grow_services (MCM under Mutators > Conquest, default on) on top of the conquest enable.

File or runtime. An outpost is half static config and half generated ini, and the split is forced by the engine, not chosen:

| Piece | Where | Why there |
|---|---|---|
| ap_service_<faction>_<role> squad sections | squad_descr_ap_services.ltx, 48 entries | SIMBOARD:create_squad resolves squad sections out of the system ini, which only loads real files at startup. A respawn entry can name only a section that already exists on disk. |
| the sim_default_<faction>_<role> profile each section wraps | vanilla character_desc | Carries the service dialogs and mechanic_mode, both parsed from XML at profile load. An arbitrary NPC cannot be granted repair or heal at runtime. |
...
| shelf caps per role | ap_outpost_stock_policy.ltx | A tuning table, read once at on_game_start. |
| the spot job (logic + beh + meet sections) | generated string, injected | A job binds to one smart's id and coordinates, and which smart is an outpost is a conquest outcome. Vanilla ships this same shape as a file (beh_gulag_jobs.ltx beh_trade_1) only because its trader smarts are fixed at build time. |
| the respawn entry pointing at the squad section | written into smart.respawn_params each scan | The engine rebuilds respawn_params from LTX on load and every tick. |

Nothing about an outpost is a file that names a smart. Files name KINDS (a squad section, a profile, a trade config, a cap); the runtime names the PLACE.

Spawn. Each conquest entry carries a .services set { [role]=true } (at most one entry). Growth is GAME-time paced: per held smart, every SERVICE_ROLL_GAME_SEC (600) of game clock earns one SERVICE_CHANCE (15%) roll -- sleeping and time skips count, and a long skip is catch-up capped at SERVICE_ROLL_MAX_CATCHUP (4) rolls per pass (the 60s real-time scan is only the pump that collects due intervals). A smart that already has its service never rolls another. Caps (both MCM): mutator_area_conquest_services_max_per_level (default 3) total service NPCs per level, and mutator_area_conquest_services_max_per_faction_role (default 1) on a given (faction, role) per level; _pick_service_role picks the LEAST-represented eligible role for the faction on the level (eligible = has a trade config and under the kind cap), tie-broken by SERVICE_ROLES order (trader first), so a faction's outposts grow trader, then barman, mechanic, medic, then round-robin as the cap allows. Picking the minimum rather than the first-eligible is load-bearing at a raised cap: first-eligible returned trader until the trader cap filled, so every outpost grew a trader and the other roles never appeared. On success the role is recorded and its respawn entry injected via xsmart_spawn.set_exclusive_spawn_section(smart, "ap_service_<role>", "ap_service_<faction>_<role>", 1) -- a faction-tagged passenger entry that rides the conquest exclusive gate (a plain shared-section entry, no .faction, would be suppressed). The smart then spawns the service squad through vanilla try_respawn. Re-injected every scan alongside the conquest entry (the engine wipes respawn_params). The 48 ap_service_<faction>_<role> squad sections (squad_descr_ap_services.ltx) wrap the vanilla sim_default_<faction>_<role> profiles (dialog, empty stock).

Service spot. Every service role gets its own runtime spot JOB (one per (smart, role), placed on a 3 m ring around the smart centre at one bearing per role and looking back at the centre, so two services never stand inside each other nor in a rank; the earlier form offset each role along the same axis with a shared look direction, which stood three of them in a line facing the same way). Static vs injected: the spawn side of an outpost is static config (squad_descr_ap_services.ltx -- SIMBOARD:create_squad resolves squad sections from the system ini, which only loads real files at startup), but the duty side cannot be static, because a job binds to one smart's id and position and which smart is an outpost is a conquest outcome. Vanilla ships the same job shape as files (beh_gulag_jobs.ltx beh_trade_1) only because its trader smarts and coordinates are fixed at build time. So the mutator formats the job ini from _SERVICE_SPOT_TEXT (smart position + id + role) and hands it to the generic injector xsmart_spawn.add_runtime_job (engine create_ini_file) -- a self-contained exclusive-style entry pushed into smart.stalker_jobs with its own ini, priority 59 (vanilla beh_trader: above camper, below surge), and a precondition admitting ONLY the outpost's own ledgered squad of that role (the vanilla trade-spot precondition admits any online NPC with sellable loot, and a profile match alone would admit a visiting sim_default_*_trader whose config the job would then re-point). Self-contained twice over: setup_logic reads job.ltx INSTEAD of the smart blob (smart_terrain.script:1378), so the meet block is inlined; and the beh scheme (axr_beh, companion + waypoint dual mode) reads its keys from the active section with actor-follow defaults, so target = waypoint (the waypoint mode switch -- without it the squad commander walks to the ACTOR: the 1.8.4 trader-follows-player bug), the anim/dist keys, and the gather/help/corpse toggles (off, the trader holds the counter) are all written inline; cross-file inheritance from beh_gulag_jobs.ltx cannot resolve inside a runtime ini. The `trade = <role cfg>` key on each spot is load-bearing twice: configure_schemes re-inits trade from it on every job (re)assignment (xr_logic.script:113), and trader_autoinject.get_trader_type reads that same ACTIVE-section key to recognise a trader at all, returning -1 without one (trader_autoinject.script:85-101) -- which is why a service parked on an ordinary walker/patrol job was skipped by every destockifier and injector in the chain, and why every role now holds a spot rather than only the trader. The smart's own continuous job selection (smart_terrain.script:611-615) seats each service, upgrading it off any lesser job it grabbed first. Jobs rebuild on every game load, so _reapply_service_entries re-ensures the entry on the scan (idempotent); decay removes it via xsmart_spawn.remove_runtime_job, which also clears the occupant's job fields so the engine reselects.

Hold. A spawned service squad is a normal migrating sim squad; without a pin it wanders off after idle_time (get_squad_target). _hold_squad sets scripted_target = smart via xsquad.acquire_squad, routing it to specific_update (holds it at the smart, keeps its jobs / animations) and ledgering it in _held_squads with the squad section as its identity. Every ledger read resolves through _resolve_held (class check + section compare), so a freed squad id recycled onto another entity is pruned instead of pinned (engine ids are pooled and reused, xrServer.cpp:873 vfFreeID). Two timed passes keep the invariant. The 10s _update_services runs _update_service per held squad: prune a gone or decayed entry, reassert the pin via _enforce_service_pin (xsquad.reassert_target), and for a service keep it on its spot job or reseat it via _enforce_service_post. The 60s _update_mutations carries the heavy lifecycle (re-apply, grow, stock) plus the adopt sweep _pin_stationed_services, which pins a service that missed its spawn-pin, a load-time event that rides the slow pass rather than the 10s one. This is NOT the ap_core_broker gulag -- the broker force-releases at a 7200s TTL and on has_jobs_for, both fatal to a permanent resident; the mutator owns the hold directly, released only on decay. A scripted_target squad reads as protected, so AP's radiant systems never script the service NPC away. scripted_target persists natively across save/load; _held_squads persists to m_data.ap_ext_smart_mutator_holds and is restored at on_game_load (stocked cleared so services re-init; the 10s pass prunes any whose smart did not restore).

Guards. On a conquest combat spawn (respawn_point_prop_section == ap_conquest), if the smart holds fewer than GUARD_QUOTA (2) guards, the squad is pinned as a guard; the rest roam (projecting the faction outward). A dead guard frees its already_spawned slot (sim_squad_scripted:on_unregister), the respawn makes another, and the next spawn re-pins -- the detail self-heals. Pinning does not starve the respawn: the per-entry num cap counts living squads until death, pinned or roaming alike.

Stock. make_trader needs an online game object, so stock waits for the service NPC to come online. On the scan _stock_services calls xcreature.make_trader(npc, cfg) for any held, online, not-yet-initialised service (trade_init only, idempotent) -- it registers the trade config but does NOT force a shelf spawn, so first stock is lazy: the vanilla on-use path fills the shelf the first time the player trades (like a hub trader), so six services coming online no longer stack six buy_supplies into one scan frame. It re-inits a service whose trade config DRIFTED off its role: every job (re)assignment re-inits trade from the job logic's trade key, default trade_generic.ltx (xr_logic.script:113; trade_init wipes on a cfg change) -- every role's spot job now carries the key, so a reassignment re-inits the RIGHT config, and _try_stock still compares cfg_ltx each scan and re-runs make_trader on a mismatch as the backstop for any pass where a service sits on some other job. Config is per-role: trader uses the faction gear config (TRADER_CFG: army->trade_military, dolg->trade_duty, csky->trade_csky_spore, ecolog->trade_ecolog_sakharov, loner->trade_stalker_owl, ...), barman / medic / mechanic use trade_generic_*. The NPC keeps its FACTION community, so ap_ext_market and every AP-family system treat it as an ordinary faction trader, and trade is relation-gated (a hostile-faction outpost serves nothing). Restock is vanilla-automatic thereafter (trade_manager.update on interaction, buy_supplies every 24h) -- with one below-hub control: the faction configs unlock supply tiers on ACTOR state (goodwill, or the heavy_pockets money achievement straight to supplies_5, trade_stalker_owl.ltx:7). Because the outpost trader uses his faction's own config, the tier the condlist picks is EXACTLY what that faction's hub trader would show the player right now (his earned standing), so left alone the outpost equals the hub. A function-level patch on trade_manager.update PREEMPTS a due restock for held outpost traders when the clamp bites. It cannot ride the original with a second buy_supplies after the fact: the engine wipes only client-REGISTERED items (CPurchaseList::process -> sell_useless_items walks inventory().m_all, InventoryOwner.cpp:600-622) and spawns the new tier ASYNC (Level().spawn_item, purchase_list.cpp:68), so a same-frame second call misses the first batch and the two tiers merge on the shelf. Instead _try_preempt_below_hub mirrors the vanilla restock gate (trade_manager.script:133-139), computes effective = clamp(min(picked - penalty, cap), 1, picked) with penalty = mutator_area_conquest_trader_supplies_penalty (default 1: one tier below hub, scales with standing) and cap = mutator_area_conquest_trader_supplies_tier (default 4: absolute ceiling; with the default penalty 1 the penalty governs and the cap only bites if the penalty is lowered), and when effective < picked stamps tm.resupply_time in vanilla's own saved shape (a fresh table, so trader_autoinject-family wrappers -- present in every GAMMA install -- and every other stamp-identity restock hook still see a real restock), runs ONE buy_supplies at the effective tier, and records current_buy_supplies; the original then still runs its buy/sell condition setup and skips its own restock on the fresh stamp (force_refresh is dropped after a preempt so a forced call cannot restock over it). When the clamp would not bite (low standing, penalty 0, unparseable tier name), nothing is stamped and the vanilla restock proceeds untouched. Hub traders and every other NPC pass through untouched. The spot job's trade key (see Service spot) is what keeps that config attached and what makes the service visible to restock-hook addons.

Stock filter. Below-hub caps the ammo/gear DEPTH; the stock policy caps the KINDS a service sells. ap_outpost_stock_policy.ltx (configs/alifeplus, DLTX-overridable) is loaded once at on_game_start via xinventory.load_policy into _stock_policy, keyed by ROLE: one [ap_outpost_stock_<role>] block each for trader, barman, mechanic and medic. Per role because a cap that suits a trader guts a specialist -- a barman's whole trade is food and drink, a medic's is medical, a mechanic's is tools and parts, and a single blanket block left each of them with a handful of items (field-measured 2026-07-26). Each role is capped generously in its own trade and hard everywhere else, and the mechanic keeps devices because detectors are a tech's business. A role whose block is missing or empty is not filtered, logged at load. After a restock is detected in _trade_update (below-hub for the trader, then all four service roles), _filter_stock is deferred ~1s (buy_supplies destroys + respawns the shelf and its items register over the next ticks, not the next frame; destockifier-style stock addons defer their own removal 1s for the same reason -- a shorter defer walks a half-spawned inventory and removes nothing) and walks the NPC's inventory: xinventory.get_category classifies each item, equipped and untouchable are skipped, ammo_* folds to the shelf key ammo_rounds_per_type (_shelf_key), and items past their rule's cap are released. Rows are a single number (a cap, no minimum -- the filter only ever removes) and the key name carries the counting rule: a plain category row is a SHELF TOTAL for that category, and the one per-type rule is ammo, counted in rounds per ammo type, because a shelf should carry several calibers and ammo is meant to dominate an outpost. Totals are what keep it lean -- vanilla lists many sections per category (77 separate meats and foods in a barman's supply list), so a per-type cap there multiplies into a warehouse: field-measured, a barman kept 164 of 170 items under per-type counting. An unlisted category is left alone, a row naming a single item section overrides that item's category row (the shared policy contract), and an absent or empty file disarms the filter loudly (info log) rather than silently. The shipped default is a field-post assortment (ammo, medical, provisions, light grenades and repair parts; no weapons, armor, artefacts, devices).

Appearance. A service is dressed from its faction pool in ap_service_visuals.ltx (configs/alifeplus, DLTX-overridable, one [ap_service_visual_<faction>] section of ordered visual paths loaded once at on_game_start into _service_visuals, mirroring the stock policy load), applied on the _stock_member path -- first stock, again after a load (stocked is cleared in _restore_holds), and on a trade-config drift, never on an ordinary scan. The pick skips any model a same-faction service already wears on the same level (_service_visual_taken; one service per outpost makes per-smart automatic, so the level is the scope that matters), starts its search at an offset from the member id so factions with room spread out, and is stored on the hold entry (entry.visual, persisted with the ledger and restored on load) so a reload and a drift re-fire keep the same look instead of re-rolling; the id-indexed slot is the fallback only when the pool is exhausted. Vanilla dresses most roles of a faction in one model (loner trader, barman and medic are all stalker_neutral2b; Duty uses stalker_dolggas2a for all four), so untouched an outpost trader looks like any other stalker of his side and two outposts on a map look like the same man. Twelve models per faction, 144 in all, exosuits excluded. Every entry is worn exclusively by generated sim_default profiles, so no named character shares a face with a service -- every model in the game that reads as a shopkeeper is already somebody (stalker_trader_1 is Sidorovich, barman\barman is the Bar barman, stalker_trader_nebo is Owl, loner_mechanic_stalker is the Cordon mechanic), which is why none of them are here and why a service looks like an armed faction member rather than a merchant. Every entry is also referenced by a vanilla character_desc profile, so the mesh ships with the base game rather than with a modpack: a candidate list built from the GAMMA tree came to 525 and only 201 resolved on a DUX-free install, since Dux's Innemurable Characters Kit contributes 3554 actor meshes of its own, and a missing model is Debug.fatal in CModelPool (ModelPool.cpp:110-124), never an invisible NPC. Within a faction the twelve are ordered balaclavas and cloth masks, then gasmasks and respirators, then helmets, then the visuals its own generic service profiles wear, then bare faces only to fill out. Six factions reach twelve on covered faces alone; monolith owns 2 and isg 4, so those fill from their own bare wardrobe, and renegade has none of its own and wears the eight bandit masks that vanilla renegade profiles already use. The 304-name candidate set was probe-verified present on both a GAMMA and a plain Anomaly install (2026-07-27). Persistence is native, not ours: set_visual_name sends GE_CHANGE_VISUAL, the server handler writes CSE_Visual::set_visual (xrServer_process_event.cpp:174), and CSE_AbstractVisual serializes it in STATE_Write, so the model survives save/load and offline/online without a re-apply. An addon that dresses the NPC from an outfit he was given (DNPCAV and kin, which cache the pre-outfit visual and restore it when the outfit leaves) holds the model between those points, deliberately -- he is wearing what he was handed, and re-asserting every scan would only flicker. Re-applying the same model costs nothing anyway: set_visual_name returns early when the visual already matches (script_game_object2.cpp:671).

Generated-ini guard. A runtime job's ini exists only in memory, so a save keeps its NAME instead -- xr_logic writes st.ini_filename (:1148) and the next load turns it back into an ini through get_customdata_or_ini_file. A name starting with "*" means a generated ini and resolves through db.dynamic_ltx (:1234-1236), the only branch of that function with no fallback: the others end at default_custom_data.ltx, this one returns nil, and initialize_obj:1262 hands it straight to configure_schemes:81, which indexes it. Two things make that reachable. xsmart_spawn.add_runtime_job wrote the "*" pointer without ever registering the ini (vanilla registers a smart's own at gulag_general.script:1331) -- fixed there. And a job NAME can vanish between versions: the 1.8.4 spot job was ap_trade_spot_<smart>, the current one is ap_service_spot_<smart>_<role>, so an NPC saved on the old name asks for something no code will ever build again. That killed the engine on a 1.8.5 save (field, 2026-07-26). ap_ext_smart_mutator hooks the function through xevent.hook and restores the missing fallback: a resolvable name returns exactly what vanilla returned, an unresolvable one loads empty logic and names the NPC and the string at WARN, and the smart's next job pass re-seats him. Renaming a runtime job is therefore a save-breaking change, and the guard is what makes it survivable.

Legacy saves. The one-service-per-outpost rule guards growth only, so a save written before it keeps every role its outposts had grown -- field logs from 2026-07-26 show 8 outposts each holding trader, barman and mechanic, standing in a rank, with one outpost consuming its map's whole service allowance so no other could grow one. The ap_v185_to_v186_clear_outpost_services changeset (ap_core_compat) empties every conquest entry's services set and drops every service hold, and the squads themselves are removed at on_game_load, because load_state runs before objects().load (alife_storage_manager.cpp:141) and no server entity exists yet. It keeps no survivor: an arbitrary one carried a legacy NPC's stock, trade config and spot forward, so all of them go and the next growth roll seats a fresh service at the corrected geometry. Both tables are mutated IN PLACE, since the mutator captures them by reference at its own load_state and dispatch order is nondeterministic (axr_main.script:272). The queued removal carries the squad's section alongside its id and compares it before deleting, the same identity guard _resolve_held applies, because freed server ids are pooled and reused (xrServer.cpp:873 vfFreeID).

Marker. The trade-spot logic section carries level_spot = trader, so the engine's stalker_generic.update_level_spot adds the standard PDA trader icon (ui_pda2_trader_location) with the vanilla trader legend, gated on the NPC's goodwill toward the actor > -1000 (hidden while his faction is hostile). This is the engine NPC spot, NOT an ap_core_map dispatch marker (that module still renders only AP-scripted squads). Vanilla removes the spot on death; on decay _remove_service_squad clears each member's spot (remove_level_spot) before removing the squad, so no icon lingers at a dissolved outpost.

Rank. The vanilla sim_default_<faction>_<role> profiles roll rank 10000-20000 (professional to master), a hub-VIP rank meant for a safe base NPC and wrong on a killable field merchant. It makes the service a needlessly strong fighter and inflates the faction-relation swing when he is killed, since relation code weights a kill by the victim's rank read from the server object (ranks.get_se_obj_rank_name -> se:rank()). _set_service_rank drops him to SERVICE_RANK (0, novice) on the server object at squad_on_npc_creation (sim_board.script:149 hands the member se_obj), the instant the member exists and before any update tick or rank read, so no code ever sees the profile rank. CInventoryOwner::SetRank writes both m_rank and the online character rank, and the engine rerolls m_rank only when NO_RANK (xrServer_Objects_ALife_Monsters.cpp:472), so the low rank survives save/load and offline/online with no window. Its own step, separate from the weapon and the stock.

Kill. A killed service NPC would drop his whole shop stock (vanilla shields hub traders with invulnerability; death_manager.decide_items_to_keep has no trade-stock concept). _on_squad_npc_death subscribes to squad_on_npc_death (sim_squad_scripted.script:493), which hands the squad object directly and fires BEFORE remove_npc strips the member's group_id. The engine npc_on_death_callback is the wrong seam here: its server-side chain (se_stalker:on_death -> squad:on_npc_death -> remove_npc) can null get_object_squad(victim) first, so a ledger lookup by squad misses and the stock drops (field-confirmed 2026-07-25). On a held service (guards exempt) the corpse inventory is released item by item (xobject.se + alife_release, the keep_item idiom). Offline death has no game object; vanilla offline corpse cleanup handles it.

Decay. _decay_one calls _release_holds_for_smart on a conquest decay: clear each ap_service_<role> respawn entry (clear_shared_spawn), release the guards back to the sim (xsquad.release_squad -- ordinary combat squads), and REMOVE the service squads from the world (xsquad.remove_squad, the vanilla SIMBOARD path). A released service would roam as a stocked shopkeeper outside every ledger-keyed protection (no loot-null on death, next restock unclamped); the service NPC exists only as the outpost's function and dissolves with it. _update_service applies the same guard/service split on the 10s pass when a hold's smart is no longer conquered: a guard is released, a service removed. The smart returns to its original spawns.

#### Swarm (exclusive spawn, mutants)

swarm_smart(smart_id, species) calls xsmart_spawn.set_exclusive_spawn(smart, "ap_swarm", species, spawn_num). Same takeover as conquest, with a mutant species instead of a faction: sets faction_controlled + faction so the engine gate (smart_terrain.script:1667) passes ONLY the swarm entry and suppresses the smart's original LTX spawns. Species comes from xsmart_spawn.SQUADS_BY_SPECIES (simulation_* sections), gated by the shared INFEST_SPECIES allowlist (descriptor allow + cause-side; see the Infestation section). The smart also gets smart.respawn_idle = cfg.mutator_area_swarm_respawn_minutes * 60 (MCM under Mutators > Swarm, default 60 game-min) so it repopulates faster while held. Occupancy stays vanilla-normal (spawn_num default 2, the total a typical LTX smart holds across its entries); elevated numbers are infest's identity (default 4).
...
Decay. cfg.mutator_area_swarm_decay_hours (default 48). Scanner checks xtime.game_sec() - swarmed_at and calls clear_exclusive_spawn on expired entries, restoring the captured pre-swarm respawn_idle; the smart returns to its original spawns.
...
Decay. Separate from conquest and swarm. cfg.mutator_area_infest_decay_hours (default 48). clear_exclusive_spawn reverts faction_controlled to nil, faction to default_faction, and removes the infest entry; the captured pre-infest respawn_idle is restored. Original LTX spawns resume on next try_respawn. Set to 0 to disable decay (permanent infestation).
