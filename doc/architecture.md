...
| ap_ext_market | Faction market: observes `ap_ext_trade.trade` output into a decaying per-faction sold-section ledger (`xttltable` counter, persisted), and at a curated faction trader's restock (one of two detection paths) injects a bounded, rank-gated (`xinventory.get_rank_chance`), role-fit, distribution-claimed, capped echo at broken condition, priced at a premium via the engine `on_get_item_cost` hook, released on a window timer. Read-side of the loot/trade/market loop; no AP money path |
...
`xinventory.load_policy(path, sections, specials_set)` is the shared LTX kernel. Each named section yields `{ entries, rules, specials }`. `entries` preserves declaration order (= BUY / LOOT priority); `rules` is the per-category hash for O(1) lookup; `specials` carries numeric keys named in `specials_set` (`profit_max`, `extras_max`, `fill_max`, `premium`, `per_trader_max`, `item_condition`, `ledger_window_hours`, ...). Consumer mods ship their own policy LTX under `configs/<mod>/<purpose>_policy.ltx`; the shape is fixed in xlibs, the values are owned by the mod. AlifePlus uses `ap_trade_policy.ltx` for trade, `ap_stash_policy.ltx` for stash, `ap_loot_policy.ltx` for corpse trim, and `ap_market_policy.ltx` for the faction market. AlifeBalance uses `ab_inventory_policy.ltx` for the inventory-balance cull pass. The market file additionally carries two non-`load_policy` sections (`[ap_market_rank_floor]`, `[ap_market_rank_chance]`) read directly into the `xinventory.get_rank_chance` ruleset; the player-rank gate has a different shape than the `{min,max}` kernel.
...
| Faction Market | `ap_ext_market.script`: observe via `xevent.hook("ap_ext_trade","trade",...)` -> faction ledger; inject at restock (Destockifier `trader_on_restock` callback, else `trade_manager.update` hook with a 0.1s defer) -> `_inject_at_trader`; price via engine `on_get_item_cost` | `ap_market_policy.ltx` `[ap_market_policy]` whitelist + `premium` / `per_trader_max` / `item_condition` / `ledger_window_hours` specials, plus `[ap_market_rank_floor]` / `[ap_market_rank_chance]` | Read-side, not a squad move. The trade hook reads `{ sold }` from each completed trade and records the sold sections under the seller's faction (`xsquad.get_squad_by_member` -> `get_community`) in a decaying `xttltable` counter (game-sec clock, persisted by `export`/`import`; a counter not a table because `ttl_table:import` calls the clock and `game_sec` AVs at `load_state`). At a curated faction trader's restock (`xdata.npc_roles` role + faction; unresolved -> skip), injects a bounded echo of that faction's recently-sold sections through the filter chain: whitelist + per-category cap, role-fit (medic = consumables, mechanic = gear/parts, general trader = all), player-rank roll (`xinventory.get_rank_chance`), cross-trader distribution claim (one section to one trader per window), per-trader cap scaled by MCM intensity. Each item spawns at `item_condition` (0.1, broken to match GAMMA) and is tagged; the `on_get_item_cost` hook prices a tagged item at `obj:cost() * premium * discount` (full base cost; condition discount bypassed) while it sits in its injecting trader (`obj:parent()` guard). A 60s timer releases items past the MCM window via `xinventory.release_item_id`; the next engine restock also purges them. No AP money path: the premium is paid by the player through the normal engine buy. Ledger persists; goods tags + distribution claims are ephemeral. |
...
Loot, trade, and market form one cycle. The corpse loot trim bounds what a stalker keeps off a body (highest-value within each category cap, the rest destroyed). The supply trader flow turns that kept surplus into cash and restocks the NPC's ammo, the in-Zone loop the NPCs run for themselves. The faction market is the read side of that flow. It records what each faction's stalkers sold, decays the record over a window, and at restock re-materialises a rank-gated, premium-priced slice into that faction's hub traders for the player to buy. The injected stock is bounded by what the faction actually sells, so it never exceeds the gear that faction fields. The market does not conserve items: every restock purges the trader's ruck (`purchase_list.cpp:20`), so the item a stalker deposited is already gone by the time the market injects, and the market re-creates a bounded echo rather than a duplicate.
