...
engine restock (a trader respawns its stock)
  -> detect (Destockifier trader_on_restock callback, else trade_manager.update hook)
       -> sell out: run the market filter over _trader_sold, release a share of each picked
          section from the fresh stock, claim it for the goods window
...
Sell-out filter. The same chain, same order, over `_trader_sold`, at the same restock before injection, against the `[ap_market_trader_sell]` whitelist (ammo via the context-free `ammo_not_equipped` category, medkits, bandages, stims; row value = sections removed per category per restock). The survivors are shuffled and at most the per-trader count of sections sells out. Per picked section `floor(held x percent / 100)` items are released (MCM trader sell-out slider, 20-95, Economy > Faction market); the floor never wipes scarce stock, and a section below the floor does not count against the caps. Tagged market goods and the trader's equipped items are never counted or removed. Each sold-out section is claimed for the goods window, which is what keeps injection off it.
