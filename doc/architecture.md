...
A held conquer smart grows into an outpost: service NPCs (trader / barman / mechanic / medic) that trade, repair, and heal, plus a small guard detail. This is conquest's growth flavor -- swarm and infest do not grow services (mutants do not build). Gated by cfg.mutator_area_conquest_grow_services (MCM under Mutators > Conquest, default on) on top of the conquest enable.

Spawn. Each conquest entry carries a .services set { [role]=true }. Growth is GAME-time paced: per held smart, every SERVICE_ROLL_GAME_SEC (600) of game clock earns one SERVICE_CHANCE (15%) roll for a role it lacks -- sleeping and time skips count, and a long skip is catch-up capped at SERVICE_ROLL_MAX_CATCHUP (4) rolls per pass (the 60s real-time scan is only the pump that collects due intervals). Caps: SERVICE_MAX_PER_LEVEL (3) total service NPCs per level, SERVICE_MAX_PER_FACTION_ROLE (1) of a given (faction, role) per level. On success the role is recorded and its respawn entry injected via xsmart_spawn.set_exclusive_spawn_section(smart, "ap_service_<role>", "ap_service_<faction>_<role>", 1) -- a faction-tagged passenger entry that rides the conquest exclusive gate (a plain shared-section entry, no .faction, would be suppressed). The smart then spawns the service squad through vanilla try_respawn. Re-injected every scan alongside the conquest entry (the engine wipes respawn_params). The 48 ap_service_<faction>_<role> squad sections (squad_descr_ap_services.ltx) wrap the vanilla sim_default_<faction>_<role> profiles (dialog, empty stock).

Trade spot. A smart with a trader service also gets a runtime trade-spot JOB: the mutator builds the job content (_TRADE_SPOT_TEXT, the vanilla beh_trade_1 shape: stand guard at the smart's position, serve trade on the trade signal, generic meet block included because setup_logic reads job.ltx INSTEAD of the smart blob, smart_terrain.script:1378) and hands it to the generic injector xsmart_spawn.add_runtime_job -- a self-contained exclusive-style entry pushed into smart.stalker_jobs with its own ini, priority 59 (vanilla beh_trader: above camper, below surge), and a precondition admitting ONLY sim_default_*_trader profiles (the vanilla trade-spot precondition admits any online NPC with sellable loot -- the wrong occupant). The smart's own continuous job selection (smart_terrain.script:611-615) seats the trader, upgrading him off any lesser job he grabbed first. Jobs rebuild on every game load, so _reapply_service_entries re-ensures the entry on the scan (idempotent); decay removes it via xsmart_spawn.remove_runtime_job, which also clears the occupant's job fields so the engine reselects.
...
Decay. _decay_one calls _release_holds_for_smart on a conquest decay: clear each ap_service_<role> respawn entry (clear_shared_spawn) and release every pinned squad (services + guards) back to the sim (xsquad.release_squad). The outpost dissolves; the smart returns to its original spawns.
