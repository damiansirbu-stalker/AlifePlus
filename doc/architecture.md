...
Stock. make_trader needs an online game object, so stock waits for the service NPC to come online. On the scan _stock_services calls xcreature.make_trader(npc, cfg, true) for any held, online, not-yet-stocked service (idempotent trade_init, so re-fire is a no-op). Config is per-role: trader uses the faction gear config (TRADER_CFG: army->trade_military, dolg->trade_duty, csky->trade_csky_spore, ecolog->trade_ecolog_sakharov, loner->trade_stalker_owl, ...), barman / medic / mechanic use trade_generic_*. The NPC keeps its FACTION community, so ap_ext_market and every AP-family system treat it as an ordinary faction trader, and trade is relation-gated (a hostile-faction outpost serves nothing). Restock is vanilla-automatic thereafter (trade_manager.update on interaction, buy_supplies every 24h) -- with one below-hub control: the faction configs unlock supply tiers on ACTOR state (goodwill, or the heavy_pockets money achievement straight to supplies_5, trade_stalker_owl.ltx:7). Because the outpost trader uses his faction's own config, the tier the condlist picks is EXACTLY what that faction's hub trader would show the player right now (his earned standing), so left alone the outpost equals the hub. A function-level patch on trade_manager.update RIDES the original restock for held outpost traders: the original runs first (vanilla stamps tm.resupply_time, the trade-UI profile stays correct, and trader_autoinject-family wrappers -- present in every GAMMA install -- see a normal trader with a real restock stamp), a restock is detected by resupply_time identity change, and the supplies pick is redone as effective = clamp(min(picked - penalty, cap), 1, picked) with penalty = mutator_area_conquest_trader_supplies_penalty (default 1: one tier below hub, scales with standing) and cap = mutator_area_conquest_trader_supplies_tier (default 5: absolute ceiling, off by default so the penalty governs). Only an effective tier below the pick pays a second buy_supplies, once per restock. Hub traders and every other NPC pass through untouched. The trade-spot logic section also carries trade = <faction cfg>: configure_schemes re-inits every stalker's trade config from that key on each job (re)assignment, defaulting to trade_generic.ltx (xr_logic.script:113), so without it a reassignment would silently reset the trader to the generic assortment; the key also classifies him as SUPPLIER for trader_autoinject.get_trader_type, so restock-hook addons (destockifiers, injectors) govern him like any other trader.
