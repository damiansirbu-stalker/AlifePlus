...
engine restock (a trader respawns its stock)
  -> detect (Destockifier trader_on_restock callback, else trade_manager.update hook)
       -> sell out: run the market filter over _trader_sold, release a random share of each
          demanded class (ammo by bullet type) from the fresh stock, at least 30% kept, claim it
