...
Hold. A spawned service squad is a normal migrating sim squad; without a pin it wanders off after idle_time (get_squad_target). _hold_squad sets scripted_target = smart via xsquad.acquire_squad, routing it to specific_update (holds it at the smart, keeps its jobs / animations) and ledgering it in _held_squads with the squad section as its identity. Every ledger read resolves through _resolve_held (class check + section compare), so a freed squad id recycled onto another entity is pruned instead of pinned (engine ids are pooled and reused, xrServer.cpp:873 vfFreeID). The scan reasserts (xsquad.reassert_target). This is NOT the ap_core_broker gulag -- the broker force-releases at a 7200s TTL and on has_jobs_for, both fatal to a permanent resident; the mutator owns the hold directly, released only on decay. A scripted_target squad reads as protected, so AP's radiant systems never script the service NPC away. scripted_target persists natively across save/load; _held_squads persists to m_data.ap_ext_smart_mutator_holds and is restored at on_game_load (stocked cleared so services re-stock; the scan prunes any whose smart did not restore).
