...
3. Walk overdue drives top-down. For each drive, walk CAUSES with matching parent drive. Per cause: RULES (per-cause enable, alignment subset, personality roll), then SCAN (filter + `find_smart_observed`). First cause that publishes wins. Stop.
